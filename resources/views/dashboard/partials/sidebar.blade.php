@php
    ...
    $linkDirectional = $isRtl ? 'flex-row-reverse text-right' : 'flex-row text-left';
    $subListPadding = $isRtl ? 'pr-6' : 'pl-6';
            ...
            <div class="flex-1 overflow-auto pr-1">
                ...
                <nav class="space-y-3">
                    ...
                    <a href="{{ url('dashboard/properties') }}"
                       class="{{ $linkBase }} {{ $linkDirectional }} {{ request()->is('dashboard/properties*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-8 w-8">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z" />
                        </svg>

                        <span class="text-base font-medium">Properties</span>
                    </a>
                    ...
                    <a href="{{ route('dashboard.amenities.index') }}"
                       class="{{ $linkBase }} {{ $linkDirectional }} {{ request()->is('dashboard/amenities*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        ...
                        <span class="text-base font-medium">Amenities</span>
                    </a>
                    ...
                    <a href="{{ url('dashboard/bookings') }}"
                       class="{{ $linkBase }} {{ $linkDirectional }} {{ request()->is('dashboard/bookings*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                            <rect x="3" y="5" width="18" height="16" rx="2" ry="2" />
                            <path d="M16 3v4M8 3v4M3 11h18" />
                        </svg>
                        <span class="text-base font-medium">Bookings</span>
                    </a>

                    {{-- Reports --}}
                    <details class="group">
                        <summary class="flex items-center gap-3 p-3 rounded-lg cursor-pointer text-gray-700 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                                <path d="M3 3v18h18" />
                        ...
                        <div class="mt-2 space-y-1 {{ $subListPadding }}">
                            <a href="{{ url('dashboard/reports/properties') }}"
                               class="block py-1 rounded-md text-sm {{ request()->is('dashboard/reports/properties') ? 'text-indigo-700 font-semibold' : 'text-gray-600 hover:text-indigo-700' }} focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white">
                                • Properties Report
                            </a>

                            <a href="{{ url('dashboard/reports/bookings') }}"
                               class="block py-1 rounded-md text-sm {{ request()->is('dashboard/reports/bookings') ? 'text-indigo-700 font-semibold' : 'text-gray-600 hover:text-indigo-700' }} focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white">
                                • Bookings Report
                            </a>
                        </div>
                    </details>
                ...
                <nav class="space-y-3">
                    <a href="{{ route('employee.dashboard.employee') }}"
                       class="{{ $linkBase }} {{ $linkDirectional }} text-gray-700 hover:bg-gray-50">
                        <svg xmlns="http://www.w3.org/2000/svg"
                             class="h-8 w-8 flex-shrink-0"
                             fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <a href="{{ route('employee.bookings.my') }}"
                       class="{{ $linkBase }} {{ $linkDirectional }} {{ request()->is('dashboard/bookings/my') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <rect x="3" y="5" width="18" height="16" rx="2" ry="2"/>
                            <path d="M16 3v4M8 3v4M3 11h18"/>
                        </svg>
                    ...
                    <a href="{{ route('employee.bookings.pending') }}"
                       class="{{ $linkBase }} {{ $linkDirectional }} {{ request()->is('dashboard/bookings/pending') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700 hover:bg-gray-50' }}">
                        ...
                        <span class="text-base font-medium">Pending Bookings</span>
                    </a>
                </nav>
                @endrole
            </div>

            {{-- Footer area: fixed at bottom of the aside --}}
            <div class="mt-4 pt-4 border-t">
                <div class="flex items-center justify-between gap-3">
                    <button
                        type="button"
                        onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'logout-modal' }))"
                        class="w-full flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-50 text-sm text-red-600"
                        aria-label="Logout">
                        {{-- logout icon --}}
